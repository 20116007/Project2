...
class ASTGenerator:
    ...
    def setup_include_paths(self, folder_path: Path):
        ...
        - goal/MIDDLE_HEADER/ (middleware headers with nested folders)
        ...
        4. Middleware headers folder (MIDDLE_HEADER with all nested subdirectories)
        ...
        # First check for MIDDLE_HEADER (exact name as mentioned by user)
        middleware_headers = self.base_path / "MIDDLE_HEADER"
        if middleware_headers.exists():
            ...
            for subdir in mw_subdirs:
                if str(subdir) not in self.include_paths:  # Avoid duplicates
                    self.include_paths.append(str(subdir))
            print(f"  Found MIDDLE_HEADER with {len(mw_subdirs)} directories (including nested)")
        
        # Also check for other common middleware folder names
        for name in ["middleware_headers", "middleware", "middleware_include", "mw_headers"]:
    ...
    def create_linkages(self):
        ...
        - Global header folders (like GLOBAL_HEADER) and nested subdirectories
        - Middleware folders (like MIDDLE_HEADER) and nested subdirectories
        ...
        for call_node, func_name, call_location in self.function_calls:
            ...
            for func_node in matching_functions:
                ...
                if "MIDDLE_HEADER" in func_file or "middleware" in func_file.lower():
    ...
    def generate_ast(self, folder_name: str):
        ...
        if not folder_path.exists():
            print(f"Error: Folder '{folder_name}' not found in {self.base_path}")
            return None
...
def main():
    ...
    print("\nAvailable folders:")
    base = Path(base_path)
    folders = [d.name for d in base.iterdir() if d.is_dir() and not d.name.startswith('.')]
    if folders:
